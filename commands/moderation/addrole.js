module.exports = {
	...
	run: async (client, message, args) => {
		...
		if (!member) {
			return message.channel.send(
				"`❌` User not found, please provide a valid user.",
			);
		}
		...
		if(member.id === message.author.id) {
			return message.channel.send(
				"`❌` You are not allowed to give yourself roles.",
			);
		}
		...
		await message.channel.send(
			`\`✔️\` Successfully gave \`${role.name}\` to **${member.user.username}**#${member.user.discriminator}`,
		).then(message.delete());
	},
};