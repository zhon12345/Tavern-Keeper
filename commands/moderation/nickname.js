module.exports = {
	...
	run: async (client, message, args) => {
		...
		if (!nickname) {
			return message.channel.send("`❌` Nickname not found, please provide a valid nickname.");
		}

		member.setNickname(nickname);
		message.channel.send(`\`✔️\` Successfully set **${member.user.tag}**'s nickname to **${nickname}**`);
	},
};
