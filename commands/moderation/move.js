...
module.exports = {
	...
	run: async (client, message, args) => {
		...
		if (member.id === message.author.id) {
			return message.channel.send("`❌` You are not allowed to move yourself.");
		}

		if (member.id === client.user.id) {
			return message.channel.send("`❌` You are not allowed to move me.");
		}
		...
		if (!member.voice.channel) {
			return message.channel.send("`❌` That member is not in a voice channel.");
		}
		...
		try {
			await member.send(
				`You have been moved from \`${member.voice.channel.name}\` to \`${channel.name}\`\n\`[Reason]\` ${Reason}`,
			);
		} catch {
			if (logchannel) {
