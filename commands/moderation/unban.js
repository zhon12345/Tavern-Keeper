...
module.exports = {
	...
	run: async (client, message, args) => {
		...
		const id = args[0];
		if (!id || isNaN(id)) {
			return message.channel.send("`❌` User ID not found, please provide a valid user ID. (eg. `450846017890549761`)");
		}

		const bannedUsers = await message.guild.fetchBans();
		const user = bannedUsers.get(id);
		if (!user) {
			return message.channel.send("`❌` User not found, please provide a valid user ID.");
		}
		...
		await message.channel
			.send(`\`✔️\` Successfully unbanned **${user.user.username}**#${user.user.discriminator}`)
			.then(message.delete());
	},
};
