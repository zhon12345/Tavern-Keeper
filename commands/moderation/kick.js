...
module.exports = {
	...
	run: async (client, message, args) => {
		...
		if (member.id === message.author.id) {
			return message.channel.send("`❌` You are not allowed to kick yourself.");
		}

		if (member.id === client.user.id) {
			return message.channel.send("`❌` You are not allowed to kick me.");
		}
		...
		if (!member.kickable) {
			return message.channel.send("`❌` An error occurred, I am unable to kick this user.");
		}
		...
		await message.channel
			.send(`\`✔️\` Successfully kicked **${member.user.username}**#${member.user.discriminator}`)
			.then(message.delete());
	},
};
