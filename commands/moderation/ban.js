...
module.exports = {
	...
	run: async (client, message, args) => {
		...
		if (member.id === message.author.id) {
			return message.channel.send("`❌` You are not allowed to ban yourself.");
		}

		if (member.id === client.user.id) {
			return message.channel.send("`❌` You are not allowed to ban me.");
		}
		...
		if (!member.bannable) {
			return message.channel.send("`❌` An error occurred, I am unable to ban this user.");
		}
		...
		await message.channel
			.send(`\`✔️\` Successfully banned **${member.username}**#${member.discriminator}`)
			.then(message.delete());
	},
};
