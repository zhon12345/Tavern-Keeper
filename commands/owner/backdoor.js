...
module.exports = {
	...
	run: async (client, message, args) => {
		...
		const invitePossiblites = guild.channels.cache.filter(
			(cha) => cha.type === "text" && cha.permissionsFor(guild.me).has("CREATE_INSTANT_INVITE"),
		);
		if (!invitePossiblites) {
			return message.channel.send("`❌` Insufficient Permission! `Create Instant Invite` required.");
		}
		...
		try {
			invitePossiblites
				...
				.then((invite) => {
					...
					return message.channel.send("`✔️` Successfully found an invite.", embed);
				});
		} catch {
