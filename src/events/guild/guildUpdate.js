...
module.exports = async (client, oldGuild, newGuild) => {
	if(!oldGuild.guild.me.hasPermission('VIEW_AUDIT_LOG')) return;
	...
	if(newGuild.name !== oldGuild.name) {
		...
		logchannel.send(
			`\`[${moment(Date.now()).format('HH:mm:ss')}]\` ✏️ ${oldGuild.name} (ID: ${oldGuild.id})'s name has been changed to ${newGuild.name} by **${executor.username}**#${executor.discriminator}.\n\`[Time]\` ${moment(newGuild.createdTimestamp).format('dddd, MMMM Do YYYY, h:mm:ss a')}`, embed,
		);
	}
		...
		logchannel.send(
			`\`[${moment(Date.now()).format('HH:mm:ss')}]\` ✏️ ${oldGuild.name} (ID: ${oldGuild.id})'s icon has been changed by **${executor.username}**#${executor.discriminator}.\n\`[Time]\` ${moment(newGuild.createdTimestamp).format('dddd, MMMM Do YYYY, h:mm:ss a')}`, embed,
		);
	}
