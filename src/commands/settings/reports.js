...
module.exports = {
	...
	run: (client, message, args) => {
		...
		if (args[0] === 'off') {
			db.set(`reports_${message.guild.id}`, null);
			message.channel.send(
				'<:vSuccess:725270799098970112> Reports has been turned off',
			).then(message.delete());
		}
		else {
			...
			db.set(`reports_${message.guild.id}`, args[0].id);
			message.channel.send(
				`<:vSuccess:725270799098970112> Reports will now be sent to ${args[0]}`,
			).then(message.delete());
		}
	},
};